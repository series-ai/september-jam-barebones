...
The genre-neutral starting point in the Back to Work Jam Kit: platform
wiring (SDK boot, lifecycles, design-unit stage, saves) plus a throwaway
demo scene, and nothing else. Start here when your game is not a workplace
sim. The kit's job-sim template (standalone mirror:
github.com/series-ai/get-to-work-jobsim) is built on exactly these
patterns and contains working systems to copy when you need them: audio
buses + settings screen, an all-time leaderboard, dialogue and story
beats, and rewarded ads (which came from the npm package
...
- **Platform systems (audio, leaderboard, dialogue, ads, ...)** → copy working implementations from the jam kit's job-sim template (github.com/series-ai/get-to-work-jobsim), or from the `@series-inc/run-game-helpers` npm package (daily rewards, IAP, quests, tutorial...). Copy the files in — never import that package at runtime.
...
**"Add audio / a leaderboard / dialogue / rewarded ads"** — copy the working system from the jam kit's job-sim template (github.com/series-ai/get-to-work-jobsim; it documents its own file list in its CLAUDE.md), or copy modules from the `@series-inc/run-game-helpers` npm package. Copy files in; never import that package at runtime.
