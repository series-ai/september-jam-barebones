...
Start here when your game is not a workplace sim. If it is, use the job-sim template ([series-ai/get-to-work-jobsim](https://github.com/series-ai/get-to-work-jobsim)) instead — and either way, it contains working systems you can copy into this one when you need them (audio + settings, leaderboard, dialogue and story beats, rewarded ads). More platform systems (daily rewards, IAP, quests, tutorial) are available as copy-in modules from the [`@series-inc/run-game-helpers`](https://www.npmjs.com/package/@series-inc/run-game-helpers) npm package. For platform details, see the [RUN.game developer docs](https://series-1.gitbook.io/rundot-docs).
...
`rundot init` runs ONCE per game (it registers the game and writes `game.config.json`); after that, iterating is just `npm run build && rundot deploy`. Deploys are **private by default** — playable by you via your profile / `rundot list-games`, but not listed on the Explore page until you deploy with `--public`.
